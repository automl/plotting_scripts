...
def plot_optimization_trace(times, performance_list, title, min_test, max_test, name_list,
    ...
    # get limits
    min_y_val = sys.maxint
    max_y_val = -sys.maxint
    ...
    for idx, performance in enumerate(performance_list):
        ...
        # Get limits
        min_y_val = min(min(min_test[idx]), min_y_val)
        max_y_val = max(max(max_test[idx]), max_y_val)
    max_trials = times[-1]
    ...
    if x_max == 0 and x_min >= 0:
        ax1.set_xlim([x_min, max_trials + 0.01*abs(max_trials)])
    elif x_max != 0 and x_min == 0:
        ax1.set_xlim([0, x_max])
    elif x_max > x_min and x_min <= 0:
        ax1.set_xlim([x_min, x_max])
    else:
        ax1.set_xlim([0, max_trials + 0.01*abs(max_trials)])
...
def main():
    prog = "python plot_test_of_best_train.py <path_to_ClassicValidationResults*.csv>"
